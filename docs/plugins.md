...
To implement a plugin in any language, use the technology-neutral [Plugin protocol
v1](plugin-protocol.md) reference. Protocol v1 remains the default generated by `plugin init` and
is the right choice for ordinary steps, executors, and helpers. Opt into [Plugin protocol
v2](plugin-protocol-v2.md) when a step needs Wuko-managed service lifetime, host-backed template
rendering, secrets, or workflow-scoped helper calls.
...
Release manifests may declare `wuko.plugin/v1` or `wuko.plugin/v2`. Wuko records that value in new
installation markers and uses it for the handshake. Markers created by older Wuko releases have no
`protocol` field and continue to mean v1.
...
A v1 installation marker omits `protocol` so that markers written by this release stay readable by
older Wuko binaries, which reject markers containing fields they do not know. A v2 marker does
record it, so an older binary declines a plugin it could not have run.

`wuko marketplace plugin init` deliberately continues to generate a v1 plugin. Authors should
change both their implementation and release manifest to v2 only after implementing its
concurrent, bidirectional message loop. See the [v2 service and callback examples](plugin-protocol-v2.md#complete-service-sequence).
...
renders with `secret` denied rather than granting it indirectly. The list is declared by the
plugin itself during `initialize`, so pinning a plugin by `sha256:` pins what it may ask for. Because a plugin step
cannot run through the host executor, plugin steps are rejected inside `executor:` blocks; because
a plugin executor exposes no filesystem, `file` and `edit` are rejected during validation inside a
plugin executor's scope.
