...
Version 1 excludes TTY interaction, streaming stdin, native task graphs, host-managed services, nested executor calls from plugin steps, filesystem access from plugin executors, and template or secret callbacks. Because a plugin step cannot run through the host executor, plugin steps are rejected inside `executor:` blocks; because a plugin executor exposes no filesystem, `file` and `edit` are rejected during validation inside a plugin executor's scope.
