...
func validateActionSourcesWithoutDependencies(steps []Step) error {
	for _, workflowStep := range steps {
		if dependencyReference(workflowStep.Uses.URL) || dependencyReference(workflowStep.Uses.Path) || dependencyReference(workflowStep.Uses.GitHub) || dependencyReference(workflowStep.Uses.Token) || dependencyReference(workflowStep.Uses.Command) {
