...
- Use `http` for structured API calls with typed JSON responses, status validation, retries, and
  timeouts. Keep authorization values in environment-backed headers. Namespaced plugin steps are
  supplied by installed plugins rather than Wuko core; consult the selected plugin's documentation
  for its configuration and lifecycle behavior.
...
- A provider context is a read-only invocation snapshot shared by top-level workflows,
  dependencies, composite actions, retries, concurrent branches, lifecycle hooks, and scheduled
  executions. GitHub activates only for `GITHUB_ACTIONS=true`; its curated fields are statically
  checked while `github.payload` remains open. A workflow that references `github` is unavailable
  outside GitHub Actions. Use `hasKey(github, "pull_request")` in Expr or
  `hasKey "pull_request" .github` in templates before accessing optional pull-request metadata,
  and never pass unvalidated event payload values into an executable or sensitive sink.
