...
// contain the one goroutine it started. The returned error deliberately does not unwrap to
// context.Canceled, so a panic is classified as a failure rather than mistaken for a cancellation.
func RecoveredPanic(recovered any) error {
	return controlpkg.RecoveredPanic(recovered)
}
