...
func TestOperationValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "copy to", raw: map[string]any{"operation": "copy_to", "container": "api", "source": "artifact", "target": "/tmp/artifact"}},
		{name: "copy from", raw: map[string]any{"operation": "copy_from", "container": "api", "source": "/tmp/result", "target": "result"}},
		{name: "templated copy", raw: map[string]any{"operation": "copy_to", "container": "{{ .vars.container }}", "source": "{{ .vars.source }}", "target": "{{ .vars.target }}"}},
		{name: "copy requires container", raw: map[string]any{"operation": "copy_to", "source": "artifact", "target": "/tmp/artifact"}, wantErr: "container is required for copy_to"},
		{name: "copy requires source", raw: map[string]any{"operation": "copy_from", "container": "api", "target": "result"}, wantErr: "source is required for copy_from"},
		{name: "copy requires target", raw: map[string]any{"operation": "copy_from", "container": "api", "source": "/tmp/result"}, wantErr: "target is required for copy_from"},
		{name: "copy isolates fields", raw: map[string]any{"operation": "copy_to", "container": "api", "source": "artifact", "target": "/tmp/artifact", "image": "alpine"}, wantErr: "image is not allowed for copy_to"},
		{name: "operation list includes copy", raw: map[string]any{"operation": "missing"}, wantErr: "copy_to"},
